...
import com.gateway.gatewayservice.user.RegisterRequest;
import com.gateway.gatewayservice.user.UserService;
...
public class KeycloakUserSyncFilter  implements WebFilter {
            ...
            return userService.validateUser(userId)
                    .flatMap(exist -> {
                        ...
                        if (registerRequest != null){
                            ...
                            log.info("user alreadyexist, skippingsync");
                            return Mono.empty();
    ...
    private RegisterRequest getUserDetails(String token) {
        try {
            String tokenWithourBearer = token.replace("Bearer","").trim();
            SignedJWT signedJWT = SignedJWT.parse(tokenWithourBearer);
